...
class Translation(object):
    ...
    def translate_en(self, text):
        ...
        translated_token = self.model.generate(**token, forced_bos_token_id=self.tokenizer.lang_code_to_id['eng_Latn'])
        en_text = self.tokenizer.batch_decode(translated_token, skip_special_tokens=True)[0]
        return en_text
    def translate_zh(self, text):
        token = self.tokenize(text)
        translated_token = self.model.generate(**token, forced_bos_token_id=self.tokenizer.lang_code_to_id['zho_Hans'])
